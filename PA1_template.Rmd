...
2.  Set system locale to Engilsh.
3.  The data is read as a data variable type data frame.  
4.  **steps** column is converted to numeric type.(missing values are coded as NA)  
5.  **date** column is converted to Date type.  
6.  **interval** column is converted to numeric type.  
```{r}
...
na_count
```
So there are **`r na_count` missing values** in our dataset.  
...
The **mean** is **`r format(mean_full_steps)`** and 
the **median** is **`r format(median_full_steps)`**.  
...
- The **mean** before populating missing values is **`r format(mean_steps)`**.  
- The **mean** after populating missing values is **`r format(mean_full_steps)`**.
  
- The **median** before populating missing values is **`r format(median_steps)`**.  
- The **median** after populating missing values is **`r format(median_full_steps)`**.  
